...
The wallet interface enables server-initiated payments with stored payment methods,
where your application controls when to charge and how much. This is different from
subscription-based systems where the payment provider manages both the schedule and amount.

**Key feature: Variable amounts**

Unlike subscription-based recurring payments (fixed amount, provider-managed schedule),
wallet-based payments let you charge ANY amount each time. The amount comes from
``payment.total`` and can be different for every charge.

**Use cases:**

* **Variable amount charges**: Usage-based billing, proration, credits, dynamic pricing
* **Flexible subscriptions**: Custom billing logic, pausing, plan changes, discounts
* **Multi-charge workflows**: Deposits, installments, split payments
* **Custom retry logic**: Your server controls when and how to retry failed charges
...
* **Currently available:**
  ...
  * DummyProvider (reference implementation for testing)

* **In development:**

  * Stripe (see `PR #467 <https://github.com/jazzband/django-payments/pull/467>`_)

Any provider supporting card tokenization can implement this interface.
...
Unlike subscription-based recurring payments (where the provider charges a fixed
amount on a schedule), wallet-based payments allow you to charge **any amount** each time.

The amount comes from ``payment.total`` and can be different for every charge:

.. code-block:: python

    # First charge: $20
    payment1 = Payment.objects.create(
        variant='payu-recurring',
        total=Decimal('20.00'),  # ← Amount 1
        currency='USD',
        wallet=wallet,
    )
    payment1.autocomplete_with_wallet()

    # Second charge: $35 (different amount!)
    payment2 = Payment.objects.create(
        variant='payu-recurring',
        total=Decimal('35.00'),  # ← Amount 2 (different!)
        currency='USD',
        wallet=wallet,
    )
    payment2.autocomplete_with_wallet()

**This flexibility enables:**

* **Usage-based billing**: Charge based on actual consumption (API calls, storage, etc.)
* **Proration**: Charge partial amounts when users change plans mid-cycle
* **Credits and discounts**: Adjust amounts dynamically based on promotions or credits
* **Multi-tier pricing**: Different amounts based on usage tiers
* **Add-ons and one-time charges**: Combine subscription + additional charges

**Contrast with subscription-based recurring:**

* PayPal Subscriptions: Provider charges fixed $20/month automatically
* Wallet-based (PayU, Stripe): You charge $20, then $35, then $42.50 - whatever you need
...
.. code-block:: python
    ...
    PAYMENT_VARIANTS = {
        ...
        # Option C: DummyProvider (testing)
        'dummy': (
...
.. code-block:: python
    ...
    def subscribe_user(request):
        ...
        provider_variant = 'payu-recurring'  # or 'stripe-recurring' when PR #467 is merged

        # Create wallet
        wallet = Wallet.objects.create(
            user=request.user,
            payment_provider=provider_variant
        )
...
* Control access to ``autocomplete_with_wallet()`` - verify user ownership before charging
* Consider encrypting tokens at rest for sensitive deployments
* Log wallet operations for audit trail and fraud detection
